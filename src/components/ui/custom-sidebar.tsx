...
export function CustomSidebar() {
  ...
  const links = [{
    label: "Home",
    href: "#hero",
    icon: <Home className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }, {
    label: "Bio",
    href: "#bio",
    icon: <User className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }, {
    label: "Projects",
    href: "#projects",
    icon: <Briefcase className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }, {
    label: "Contact",
    href: "#contact",
    icon: <MessageSquare className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }];
  const socialLinks = [{
    label: "GitHub",
    href: "https://github.com/biditraj",
    icon: <Github className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }, {
    label: "Instagram",
    href: "https://www.instagram.com/shutup.bidit?igsh=MmRtajA4ZTR1ZnI=",
    icon: <Instagram className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }, {
    label: "Email",
    href: "mailto:biditraj@gmail.com",
    icon: <Mail className="text-foreground/80 h-5 w-5 flex-shrink-0" />
  }];
        ...
        <Sidebar open={open} setOpen={setOpen}>
          <SidebarBody className="justify-between gap-10 bg-background/20 backdrop-blur-xl border-r border-white/10">
            ...
            <div className="flex flex-col gap-2 mt-6">
              ...
              {socialLinks.map((link, idx) => <div key={idx}>
                  <SidebarLink link={link} className="hover:bg-primary/10 rounded-md px-2 transition-all duration-300" />
                </div>)}
            </div>
          </SidebarBody>
